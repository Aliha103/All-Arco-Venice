...
const PERMISSIONS: Permission[] = [
  ...
  },
  {
    ...
    description: "Remove bookings",
    category: "bookings",
    riskLevel: "high",
    dependencies: ["bookings:view", "bookings:edit"],
    conflicts: [],
    auditRequired: true,
    temporaryGrant: false,
    resourceAccess: ["bookings"],
  ...
  },
  {
    ...
    category: "bookings",
    riskLevel: "high",
    dependencies: ["bookings:view", "financial:view"],
    conflicts: [],
    auditRequired: true,
    temporaryGrant: false,
    resourceAccess: ["bookings", "payments"],
    conditions: [],
  },
  ...
  // Financial Operations
  {
    ...
    auditRequired: true,
    temporaryGrant: false,
    resourceAccess: ["payments", "revenue"],
    conditions: [],
  },
  {
    ...
    category: "financial",
    riskLevel: "medium",
    dependencies: ["financial:view"],
    conflicts: [],
    auditRequired: true,
    temporaryGrant: false,
    resourceAccess: ["payments", "revenue", "reports"],
    conditions: [],
  },
  {
    key: "financial:edit_pricing",
    label: "Edit Pricing",
    description: "Modify pricing settings",
    category: "financial",
    riskLevel: "high",
    dependencies: ["financial:view"],
    conflicts: [],
    auditRequired: true,
    temporaryGrant: false,
    resourceAccess: ["pricing"],
    conditions: [],
  },
  {
    key: "financial:process_payments",
  ...
  },
  {
    ...
    description: "Modify team member permissions",
    category: "team",
    riskLevel: "high",
    dependencies: ["team:view"],
    conflicts: [],
  ...
  },
  {
    ...
    category: "team",
    riskLevel: "critical",
    dependencies: ["team:view", "team:edit"],
    conflicts: [],
    auditRequired: true,
    temporaryGrant: false,
    resourceAccess: ["team"],
  ...
  // Security & Audit
  {
    ...
    auditRequired: true,
    temporaryGrant: false,
    resourceAccess: ["audit"],
    conditions: [],
  },
  {
    key: "security:configure",
  ...
  {
    key: "monitoring:real_time",
    label: "Real-time Monitoring",
    description: "Access real-time monitoring",
    category: "monitoring",
    riskLevel: "medium",
    dependencies: ["monitoring:view"],
    conflicts: [],
...
const AdvancedTeamManagement: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "members" | "roles" | "permissions" | "analytics" | "security"
  >("members");
  ...
  const [memberCreationSteps] = useState([
    ...
    },
    {
      ...
      description: "Access level and role assignment",
    },
    {
      id: 3,
      title: "Security Settings",
      description: "MFA, biometrics, and restrictions",
    },
    {
      id: 4,
      title: "Review & Create",
      description: "Verify details and create member",
    },
  ]);
  ...
  const [roleCreationSteps] = useState([
    {
      id: 1,
      title: "Basic Details",
      description: "Role name, description, and color",
    },
    {
      id: 2,
      title: "Permissions",
      description: "Define role permissions and restrictions",
    },
    {
      id: 3,
    ...
    },
    {
      id: 4,
      title: "Review & Create",
      description: "Verify and create the role",
    },
  ]);
  ...
  const [newRole, setNewRole] = useState({
    ...
    conditionalPermissions: [] as Array<{
      condition: string;
      permissions: string[];
    }>,
    isActive: true,
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const action =
        KEYBOARD_SHORTCUTS[combo as keyof typeof KEYBOARD_SHORTCUTS];
      if (action) {
  ...
  const saveEditedMember = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/team/members/${selectedMember.id}`,
        {
          method: "PATCH",
          headers: {
  ...
  const handleCreateMember = async () => {
    ...
    try {
      ...
      if (response.ok) {
        await response.json();
        setCreateSuccess(
          `Team member ${newMember.firstName} ${newMember.lastName} created successfully!`
        );
  ...
  const handleExportData = async () => {
    try {
      ...
      a.href = url;
      a.download = `team-management-export-${new Date()
        .toISOString()
        .slice(0, 10)}.json`;
  ...
  const handleResetPassword = async (memberId: string) => {
    if (
      !window.confirm("Are you sure you want to reset this member's password?")
    ) {
      return;
    }
  ...
  const handleBulkAction = async (action: string) => {
    ...
    if (action === "delete") {
      ...
      const hasSuperUser = selectedMemberObjects.some((member) =>
        isSuperUser(member)
      );
      ...
      if (hasSuperUser) {
        setNotifications((prev) => [
          ...prev,
          {
            ...
            title: "Cannot Delete Super User",
            message:
              "Super user admin@allarco.com cannot be deleted. Please deselect and try again.",
            timestamp: new Date(),
            read: false,
          },
        ]);
  ...
  const filteredMembers = useMemo(() => {
    return safeArray(teamMembers).filter((member) => {
      ...
      const matchesAccessLevel =
        filterAccessLevel === "all" || member.accessLevel === filterAccessLevel;
      ...
      return (
        matchesSearch && matchesRole && matchesAccessLevel && matchesRiskLevel
      );
    });
  }, [
    teamMembers,
          ...
          <div className="title-section">
            ...
            <div className="team-stats">
              ...
              <div className="stat-chip">
                ...
                <span>
                  {safeArray(teamMembers).filter((m) => m.isActive).length}{" "}
                  Active
                </span>
              </div>
              <div className="stat-chip">
              ...
              <span
                className={getSystemHealthColor(
                  typeof systemHealth.disk === "object"
                    ? (systemHealth.disk as any)?.usage || 0
                    : systemHealth.disk
                )}
              >
                {typeof systemHealth.disk === "object"
                  ? `${(systemHealth.disk as any)?.usage || 0}/${
                      (systemHealth.disk as any)?.available || 0
                    }`
                  : `${systemHealth.disk}%`}
              </span>
            </div>
        ...
        <div className="members-content">
          ...
          <div className="members-grid enhanced">
            {filteredMembers.map((member, index) => (
              <div
                key={member.id}
                className="member-card enhanced"
                style={
                ...
                <div className="member-header">
                  ...
                  <div className="member-info">
                    <h3 className="member-name">
                      ...
                      {member.riskScore > 75 && (
                        <AlertTriangle className="risk-warning" />
                      )}
                      {member.aiInsights?.anomalies.length > 0 && (
                        <Brain className="ai-warning" />
                      )}
                    </h3>
                  ...
                  <div className="member-actions">
                    ...
                    >
                      {showSensitiveInfo[member.id] ? (
                        ...
                        <Eye className="action-icon opacity-50" />
                      )}
                    </button>
                    <button
                      onClick={() =>
                      ...
                      className={`action-btn ${
                        member.isActive ? "danger" : "success"
                      }`}
                        ...
                        <div className="dropdown-menu enhanced">
                          ...
                          {isSuperUser(member) ? (
                            <div
                              ...
                              title="Super user cannot be deleted"
                            >
                  ...
                  <div className="member-details enhanced">
                    <div className="member-stats">
                      <div className="stat-item">
                        ...
                        <span
                          className={`stat-value ${getRiskScoreColor(
                            member.riskScore
                          )}`}
                        >
                          {member.riskScore}%
                    ...
                    <div className="performance-metrics">
                      ...
                      <div className="metric-item">
                        ...
                        <span>
                          Avg Time:{" "}
                          {member.performanceMetrics?.averageResponseTime || 0}
                          ms
                        </span>
                      </div>
                      <div className="metric-item">
                        ...
                        <span>
                          Error Rate:{" "}
                          {member.performanceMetrics?.errorRate || 0}%
                        </span>
                      </div>
                    </div>

                    {/* Security Events */}
                    {member.securityEvents &&
                      member.securityEvents.length > 0 && (
                    ...
                    {/* AI Recommendations */}
                    {member.aiInsights?.recommendations &&
                      member.aiInsights.recommendations.length > 0 && (
                ...
                <div className="role-advanced-info">
                  ...
                  <div className="role-restrictions">
                    ...
                    <div className="restriction-items">
                      ...
                      {role.restrictions?.deviceRestrictions
                    ...
                    <span className="stat-value">
                      {Math.round(
                        teamMembers
                          ...
                          (teamMembers.filter((m) => m.role.id === role.id)
                            .length || 1)
                      )}
                      %
                    </span>
              ...
              <div className="permissions-grid enhanced">
                {PERMISSIONS.filter((p) => p.category === category).map(
                  (permission) => (
                    <div
                      key={permission.key}
                      className="permission-card enhanced"
                    >
                ...
                <div className="risk-distribution">
                  <div className="risk-item low">
                    <span className="risk-count">
                      {teamMembers.filter((m) => m.riskScore <= 25).length}
                    </span>
                  ...
                  <div className="risk-item medium">
                    <span className="risk-count">
                      {
                        teamMembers.filter(
                          (m) => m.riskScore > 25 && m.riskScore <= 50
                        ).length
                      }
                    </span>
                  ...
                  <div className="risk-item high">
                    <span className="risk-count">
                      {
                        teamMembers.filter(
                          (m) => m.riskScore > 50 && m.riskScore <= 75
                        ).length
                      }
                    </span>
                  ...
                  <div className="risk-item critical">
                    <span className="risk-count">
                      {teamMembers.filter((m) => m.riskScore > 75).length}
                    </span>
              ...
              <div className="insight-content">
                <div className="permission-stats">
                  <div className="stat">
                    <span className="stat-value">
                      {
                        PERMISSIONS.filter((p) => p.riskLevel === "critical")
                          .length
                      }
                    </span>
                  ...
                  <div className="stat">
                    <span className="stat-value">
                      {teamMembers.reduce(
                        (sum, m) =>
                          sum +
                          m.role.permissions.length +
                          m.customPermissions.length,
                        0
                      )}
                    </span>
          ...
          <div className="security-overview">
            <div className="security-metric">
              ...
              <span className="metric-value">
                {
                  teamMembers.filter((m) => m.biometricData?.fingerprintEnabled)
                    .length
                }
              </span>
            ...
            <div className="security-metric">
              ...
              <span className="metric-value">
                {Math.round(
                  teamMembers.reduce((sum, m) => sum + m.riskScore, 0) /
                    teamMembers.length
                )}
                %
              </span>
                  ...
                  {!alert.resolved && (
                    <div className="alert-actions">
                      {alert.actions.map((action, idx) => (
                        <button
                          key={idx}
                          className="futuristic-btn small secondary"
                        >
                          {action}
                        </button>
                      ))}
          ...
          <div className="modal-content large step-wizard">
            <div className="modal-header">
              <h2 className="modal-title">
                ...
                Create Team Member - Step {createMemberStep} of{" "}
                {memberCreationSteps.length}
              </h2>
              <p className="modal-subtitle">
                {memberCreationSteps[createMemberStep - 1]?.description}
              </p>
                ...
                {memberCreationSteps.map((step) => (
                  <div
                    ...
                    className={`step-indicator ${
                      createMemberStep === step.id ? "active" : ""
                    } ${createMemberStep > step.id ? "completed" : ""}`}
                    onClick={() => goToStep(step.id)}
                    style={{ cursor: "pointer" }}
                  >
                    <div className="step-number">
                      {createMemberStep > step.id ? (
            ...
            <div className="step-content">
              {createMemberStep === 1 && (
                <div className="step-form">
                  ...
                  <p className="step-description">
                    Enter the team member's personal details and contact
                    information.
                  </p>
              ...
              {createMemberStep === 2 && (
                <div className="step-form">
                  ...
                  <p className="step-description">
                    Assign a role and configure access permissions for the team
                    member.
                  </p>
                  ...
                  <div className="form-group">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewMember({ ...newMember, roleId: e.target.value })
                      }
                      required
                    >
                      ...
                      {[
                        {
                          ...
                          description:
                            "Basic permissions with role restrictions",
                        },
                        {
                          value: "full",
                          label: "Full Access",
                          description:
                            "Complete access to all role permissions",
                        },
                        {
                          value: "read_only",
                          label: "Read Only",
                        ...
                        },
                        {
                          value: "custom",
                          label: "Custom",
                          description: "Define specific permissions manually",
                        },
                      ].map((level) => (
                        <label
                          key={level.value}
                          className="radio-option enhanced"
                        >
                          <input
                            type="radio"
                            name="accessLevel"
                              ...
                              <div className="permission-checkboxes">
                                {PERMISSIONS.filter(
                                  ...
                                  .map((permission) => (
                                    <label
                                      key={permission.key}
                                      className="checkbox-option enhanced"
                                    >
                                      <input
                                        type="checkbox"
                                        checked={newMember.customPermissions.includes(
                                          permission.key
                                        )}
                                        onChange={(e) => {
                                          if (e.target.checked) {
                                            ...
                                            setNewMember({
                                              ...newMember,
                                              customPermissions:
                                                newMember.customPermissions.filter(
                                                  (p) => p !== permission.key
                                                ),
                                            });
                                          }
                                      ...
                                      <span className="checkbox-label">
                                        {permission.label}
                                        <span
                                          className={`risk-indicator ${permission.riskLevel}`}
                                        >
                                          {permission.riskLevel}
                                        </span>
                                      </span>
              ...
              {createMemberStep === 3 && (
                <div className="step-form">
                  ...
                  <p className="step-description">
                    Configure advanced security options and access restrictions.
                  </p>
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewMember({
                          ...newMember,
                          locationRestrictions: e.target.value,
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewMember({
                          ...newMember,
                          deviceRestrictions: e.target.value,
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewMember({
                          ...newMember,
                          timeBasedAccess: e.target.value,
                        })
                      }
                    />
                  </div>
              ...
              {createMemberStep === 4 && (
                <div className="step-form">
                  ...
                  <p className="step-description">
                    Review all details before creating the team member.
                  </p>
                    ...
                    <div className="review-section">
                      ...
                      <div className="review-grid">
                        <div className="review-item">
                          ...
                          <span className="review-value">
                            {roles.find((r) => r.id === newMember.roleId)
                    ...
                    <div className="review-section">
                      ...
                      <div className="review-grid">
                        <div className="review-item">
                          ...
                          <span className="review-value">
                            {newMember.requiresMFA ? "Yes" : "No"}
                          </span>
                        </div>
                        <div className="review-item">
                          <span className="review-label">Biometric Auth:</span>
                          <span className="review-value">
                            {newMember.biometricEnabled
                              ? "Enabled"
                              : "Disabled"}
                          </span>
                        </div>
          ...
          <div className="modal-content large step-wizard">
            <div className="modal-header">
              <h2 className="modal-title">
                ...
                Create Role - Step {createRoleStep} of{" "}
                {roleCreationSteps.length}
              </h2>
              <p className="modal-subtitle">
                {roleCreationSteps[createRoleStep - 1]?.description}
              </p>
              ...
              <div className="step-progress">
                {roleCreationSteps.map((step) => (
                  <div
                    key={step.id}
                    className={`step-indicator ${
                      createRoleStep === step.id ? "active" : ""
                    } ${createRoleStep > step.id ? "completed" : ""}`}
                    onClick={() => goToRoleStep(step.id)}
                    style={{ cursor: "pointer" }}
                  >
                    <div className="step-number">
                      {createRoleStep > step.id ? (
                        <CheckCircle className="step-icon" />
                      ) : (
                        step.id
                      )}
            ...
            <div className="step-content">
              {createRoleStep === 1 && (
                <div className="step-form">
                  ...
                  <p className="step-description">
                    Define the role's basic information and visual appearance.
                  </p>
                  ...
                  <div className="form-group">
                    ...
                    />
                    <span className="form-hint">
                      Internal role identifier (lowercase, underscore allowed)
                    </span>
                  </div>
              ...
              {createRoleStep === 2 && (
                <div className="step-form">
                  ...
                  <p className="step-description">
                    Select the permissions this role will have access to.
                  </p>
                          ...
                          <div className="category-header">
                            ...
                            >
                              {PERMISSIONS.filter(
                                ...
                                newRole.permissions.includes(p.key)
                              )
                                ? "Deselect All"
                                : "Select All"}
                            </button>
              ...
              {createRoleStep === 3 && (
                <div className="step-form">
                  ...
                  <p className="step-description">
                    Configure security and compliance settings for this role.
                  </p>
              ...
              {createRoleStep === 4 && (
                <div className="step-form">
                  ...
                  <p className="step-description">
                    Review all role details before creating.
                  </p>
                              ...
                              <div key={category} className="category-summary">
                                ...
                                <div className="permission-tags">
                                  {categoryPerms.map((perm) => (
                                    <span
                                      key={perm.key}
                                      className={`permission-tag ${perm.riskLevel}`}
                                    >
                                      {perm.label}
                                    </span>
                                  ))}
                    ...
                    <div className="review-section">
                      ...
                      <div className="review-grid">
                        <div className="review-item">
                          <span className="review-label">MFA Required:</span>
                          <span className="review-value">
                            {newRole.complianceSettings.requiresMFA
                              ? "Yes"
                              : "No"}
                          </span>
                        </div>
                        <div className="review-item">
                          ...
                          <span className="review-value">
                            {newRole.complianceSettings.sessionTimeout}s
                          </span>
                        </div>
                        <div className="review-item">
                          <span className="review-label">Audit Level:</span>
                          <span className="review-value">
                            {newRole.complianceSettings.auditLevel}
                          </span>
                        </div>
                      </div>
                    </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content extra-large">
            <div className="modal-header">
              ...
              <button onClick={closeMemberDetailsModal} className="modal-close">
                <XCircle className="close-icon" />
              </button>
            </div>
                ...
                <div className="member-basic-info">
                  ...
                  <div className="member-status">
                    ...
                    <span
                      className="role-badge"
                      style={{
                        backgroundColor: selectedMember.role.color + "20",
                        color: selectedMember.role.color,
                      }}
                    >
                      ...
                      <div className="form-section">
                        ...
                        <div className="form-grid">
                          ...
                          <div className="form-group">
                            ...
                            <select
                              value={
                                editMemberData.isActive ? "active" : "inactive"
                              }
                              onChange={(e) =>
                                setEditMemberData({
                                  ...editMemberData,
                        ...
                        <div className="permissions-grid">
                          {PERMISSIONS.map((permission) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={editMemberData.customPermissions.includes(
                                  permission.key
                                )}
                                onChange={(e) => {
                                  if (e.target.checked) {
                                    setEditMemberData({
                                      ...editMemberData,
                                      customPermissions: [
                                        ...editMemberData.customPermissions,
                                        permission.key,
                                      ],
                                    });
                                  } else {
                                    setEditMemberData({
                                      ...editMemberData,
                                      customPermissions:
                                        editMemberData.customPermissions.filter(
                                          (p) => p !== permission.key
                                        ),
                                    });
                                  }
                    ...
                    <div className="member-info-display">
                      ...
                      <div className="info-section">
                        ...
                        <div className="permissions-display">
                          <div className="permission-category">
                            <h4>Role Permissions</h4>
                            <div className="permission-tags">
                              {selectedMember.role.permissions.map(
                                (permission) => (
                                  <span
                                    key={permission}
                                    className="permission-tag"
                          ...
                          {selectedMember.customPermissions &&
                            selectedMember.customPermissions.length > 0 && (
                              <div className="permission-category">
                                <h4>Custom Permissions</h4>
                                <div className="permission-tags">
                                  {selectedMember.customPermissions.map(
                                    (permission) => (
                                      <span
                                        key={permission}
                                        className="permission-tag custom"
                                      >
                                        {PERMISSIONS.find(
                                          (p) => p.key === permission
                                        )?.label || permission}
                      ...
                      <div className="info-section">
                        ...
                        <div className="activity-stats">
                          ...
                          <div className="stat-card">
                            <div className="stat-value">
                              {selectedMember.performanceMetrics
                                ?.tasksCompleted || 0}
                            </div>
