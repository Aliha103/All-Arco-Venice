    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <section className="px-4 py-6">
        <div className="max-w-6xl mx-auto">
          <div className="bg-white rounded-xl border border-gray-200 p-6 transition-all duration-300 hover:shadow-md hover:border-gray-300">
            <div className="flex flex-col xl:flex-row xl:items-center gap-4">
              {/* Top Row: Host Info and Price */}
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center transition-transform duration-200 hover:scale-110 cursor-pointer">
      ...
      <section id="booking-section" className="py-16 px-4 max-w-4xl mx-auto">
        <Card className="transition-all duration-300 hover:shadow-lg"><CardContent className="p-8 space-y-8">
          ...
          <Button disabled={Object.keys(validationErrors).length>0||!checkIn||!checkOut} className="w-full transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] transform" asChild><a href="/api/login" className="flex items-center justify-center"><Lock className="w-4 h-4 mr-2 transition-transform duration-200"/>Reserve securely</a></Button>
        </CardContent></Card>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div key={title} className="bg-gray-50 rounded-xl p-6 flex space-x-4 items-start transition-all duration-300 hover:shadow-md hover:bg-gray-100 hover:-translate-y-1 cursor-pointer">
                <div className="w-12 h-12 bg-white rounded-lg flex items-center justify-center shadow transition-transform duration-200 group-hover:scale-110">{icon}</div>
                <div><h3 className="font-semibold text-gray-900 mb-1 transition-colors duration-200">{title}</h3><p className="text-sm text-gray-600 transition-colors duration-200">{desc}</p></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 px-4 bg-gray-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            ...
            <div key={name} className="bg-white rounded-xl p-6 shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 cursor-pointer">
              <div className="flex items-center mb-4">
                <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center mr-3 transition-transform duration-200 hover:scale-110">
                  <span className="text-blue-600 font-semibold text-sm">{name[0]}</span>
                </div>
                <div>
                  <h4 className="font-semibold text-gray-900 transition-colors duration-200">{name}</h4>
                  <p className="text-sm text-gray-600 transition-colors duration-200">{location}</p>
                </div>
              </div>
              <div className="flex mb-3">{[1,2,3,4,5].map(n=> <Star key={n} className="w-4 h-4 text-yellow-400 fill-current transition-all duration-200 hover:scale-110" />)}</div>
              <p className="text-gray-700 text-sm transition-colors duration-200">"{text}"</p>
            </div>
          ))}
        </div>
      </section>
...
function ChatWidget(){
  ...
  return open?(
    ...
    <button onClick={()=>setOpen(true)} className="fixed bottom-6 right-6 w-16 h-16 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg flex items-center justify-center z-50 transition-all duration-300 hover:scale-110 active:scale-95 hover:shadow-xl"><MessageCircle className="w-6 h-6 transition-transform duration-200"/></button>
  )
}