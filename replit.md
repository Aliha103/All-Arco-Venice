...
## Changelog
- June 29, 2025. COMPLETED: Ultra-compact no-scroll mobile calendar optimization - Updated all calendar components to use full available width (w-full max-w-full) with overflow-hidden to prevent horizontal scrolling on mobile devices. Enhanced both SmoobuCalendar component and admin dashboard calendar with optimal screen-fit display that eliminates horizontal scroll issues while preserving standard width for tablets and desktops. Mobile calendars now provide perfect fit-to-screen experience with preserved 100ms real-time refresh functionality and complete booking management capabilities
