...
## Changelog
- June 29, 2025. COMPLETED: Screen-fit mobile calendar optimization - Updated all calendar components to use full screen width (w-screen) for mobile devices to maximize available space. Enhanced both SmoobuCalendar component and admin dashboard calendar with consistent screen-fit mobile display while preserving standard width for tablets and desktops. Mobile calendars now utilize complete screen real estate for optimal viewing with preserved 100ms real-time refresh functionality and complete booking management capabilities
