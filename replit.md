...
## Changelog
- June 29, 2025. COMPLETED: Complete calendar system overhaul with click-based interaction and minimum booking requirements. Replaced drag interface with professional click/tap popup system offering "Block Dates" or "Manual Booking" options. Enforced minimum booking requirement from check-in to check-out next day. Fixed broken pipe display between dates with seamless continuous styling. Added admin price control and payment method selection for manual bookings. Implemented clean color-coded booking sources: green (direct website), red (Airbnb), blue (Booking.com), gray (blocked periods). Created comprehensive booking management with real-time calendar updates and professional responsive design
