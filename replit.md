...
## Changelog
- June 29, 2025. COMPLETED: Ultra-compact mobile calendar optimization - Reduced all calendar components to 40% width (w-[40%]) for mobile screens only to maximize space efficiency and eliminate all horizontal scrolling. Updated main SmoobuCalendar component, admin dashboard timeline tab, and admin-dashboard-fixed calendar tab with responsive overflow handling. Increased mobile cell height to 60px (h-15) for better touch targets and content visibility. Added custom h-15 CSS utility class and updated JavaScript booking span calculations. Calendar maintains full width on tablets and desktops while providing ultra-compact mobile experience across all sections including both admin dashboard versions
