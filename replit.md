...
## Changelog
- June 29, 2025. COMPLETED: Full-width mobile calendar optimization - Updated all calendar components to use 100% width (w-full) for mobile screens to maximize readability and touch interaction. Enhanced both SmoobuCalendar component and admin dashboard calendar with full-width mobile display while maintaining responsive design for tablets and desktops. Mobile calendars now utilize entire available screen width for optimal user experience with preserved 100ms real-time refresh functionality and complete booking management capabilities
