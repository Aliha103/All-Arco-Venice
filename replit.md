...
## Changelog
- June 29, 2025. COMPLETED: Ultra-compact mobile calendar optimization - Reduced calendar width to 45% (w-[45%]) for mobile screens only to maximize space efficiency and eliminate all horizontal scrolling. Increased mobile cell height to 60px (h-15) for better touch targets and content visibility. Added custom h-15 CSS utility class. Updated JavaScript booking span calculations to match new 60px mobile cell height. Calendar maintains full width on tablets and desktops while providing ultra-compact mobile experience for optimal viewing on small screens
