...
## Changelog
- June 29, 2025. COMPLETED: Ultra-compact mobile calendar optimization - Updated all calendar components to use 50% width (w-[50%]) for mobile screens only to provide compact viewing while maintaining usability. Enhanced both SmoobuCalendar component and admin dashboard calendar with consistent 50% mobile width while preserving full width for tablets and desktops. Mobile calendars now provide ultra-compact experience with preserved 100ms real-time refresh functionality and complete booking management capabilities
